...
* [查询优化之页面静态化](#查询优化之页面静态化)
  ...
  * [CDN](#cdn)
    ...
    * [CDN优化效果](#cdn优化效果)
    * [CDN深入](#cdn深入)
   	 * [cache controll响应头](#cache-controll响应头)
   	 * [浏览器三种刷新方式](#浏览器三种刷新方式)
     * [自定义缓存策略](#自定义缓存策略)
     * [静态资源部署策略](#静态资源部署策略)
  * [全页面静态化](#全页面静态化)
...
3. **摘要作为文件名**：比如`45edw.js`，会同时存在新老两个版本，方便回滚。推荐第三种方法

**策略二**
1. 对应静态资源保持生命周期内不会变，max-age可设置很长，无视失效更新周期。
...
**策略三**
1. 动态请求静态化json资源（.json文件）推送到CDN，添加版本号，客户端每次可以先展示CDN缓存的json同时异步AJAX请求后端判断版本号。
2. 商品出现违规信息，需要紧急下架，依靠**异步AJAX请求**后端获取对应资源状态，如果资源状态是紧急下架，覆盖CDN对应资源。
